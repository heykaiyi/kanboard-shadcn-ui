...
│   └── SendLogger.php
├── Locale/zh_TW/translations.php     這個外掛引入的字串
├── Template/
...
- **繁體中文。** `Locale/zh_TW` 涵蓋這個外掛引入的字串——側邊欄自己的標籤，加上 `Calendar`
  與 `Gantt`，那是側邊欄自己畫的而核心沒有對應條目。其餘全部來自 Kanboard 自己的 `zh_TW`，
  它是完整的而且用台灣的詞（專案、儲存、看板）。Calendar 與 Gantt 這兩個外掛只附簡體中文，
  所以它們自己的設定畫面需要各自的 `zh_TW` 檔——那個住在那些外掛裡，不在這裡。
